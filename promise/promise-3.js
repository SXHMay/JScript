...
class Promise {
  constructor (executor) {
    ...
    let resolve = (value) => { // 成功
      if (value instanceof Promise ) {// 只能判断是不是自己的promise
  ...
  then(onFulfilled, onRejected){
    ...
    return this.then((data)=>{// finally方法之后还可以调用then，则finally实际上也是一个then
      // 调用resolve确保callback中的promise执行完成
...
Promise.resolve = function(data){// 如果data是一个promise，会等到它执行完成
  return new Promise((resolve,reject)=>{
    resolve(data);
  })
}

Promise.reject = function(err){// 如果err是一个promise，会直接向下抛出，没有等待的效果
  return new Promise((resolve,reject)=>{
    reject(err);
  })
}
...
Promise.all = function(promises){// 全部的成功才成功，有一个失败就走失败的逻辑
  return new Promise((resolve, reject)=>{
    let arr = [];
    let index = 0;
    let processData = (currentValue, i) => {
      arr[i] = currentValue;
      if ( ++i == promises.length) {// 当循环的个数与参数个数相等时就直接转成成功状态
        resolve(arr);
      }
    }
    for (let i=0; i<promises.length; i++) {
      let currentValue = promises[i];
      if (isPromise(currentValue)) {// 如果是promise，执行它的then方法看成功还是失败，成功的话存储成功后的值，失败的话就直接抛出错误，不再继续执行
        currentValue.then((data)=>{
          processData(data, i);
        }, reject)
        
      } else {// 不是promise直接存储参数值
        processData(data, i);
      }
    }
  })
